...
// Implementação da interface e métodos de faturaSource
// Será usado em pgx.CopyFromSource
// Antes de fazer o copyfrom, ele simula a criação de uma source compatível
...
func workerCopyFrom(ctx context.Context, pool *pgxpool.Pool, dataChan <-chan []Fatura, wg *sync.WaitGroup) {
	...
	for faturas := range dataChan {
		...
		// Popula a interface compatível (CopyFrom) com os dados
		fs := &faturaSource{faturas: faturas}
...
func main() {
	...
	for i := 0; i < numWorkers; i++ {
		...
		go workerCopyFrom(context.Background(), pool, dataChan, &wg)
	}
	...
	log.Printf("Inserção em fluxo (CopyFrom) concluída com sucesso em %v", duration)
}
